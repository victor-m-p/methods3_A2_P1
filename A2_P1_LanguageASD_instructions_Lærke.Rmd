...
lm3 <- lmer(CHI_MLU ~ VISIT*Diagnosis.x + (1 + ADOS2| SUBJ), new_df)


- ExpressiveLangRaw or Mullenraw
- Socialization
- ADOS
...
quadratic_m3.3 <- lmer(CHI_MLU ~ VISIT + VISIT2 + Diagnosis + VISIT*Diagnosis + (1 + Diagnosis + MOT_MLU | SUBJ), df)  # So, it turns our, that this model is actually better
...
a <- ggplot(df, aes(VISIT, CHI_MLU, colour = Diagnosis)) + 
  geom_point() + 
  geom_smooth(se = FALSE, method = "lm", formula = y ~ x + poly(x, 2)) +
  ggtitle('Development of MLU for children with ASD and TD over time')
